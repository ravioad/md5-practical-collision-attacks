...
def adjustPDF(contents):
  ...
  contents = contents[:startStartXref] + ("%i" % startXREF).encode("ascii") + contents[endStartXref:]
...
# Build explicit mapping for the bytes template
fmt = {
    b'COUNT1': COUNT1,   # int for %i
    b'COUNT2': COUNT2,   # int for %i
    b'KIDS1' : KIDS1,    # bytes for %s
    b'KIDS2' : KIDS2,    # bytes for %s
}
print("COUNT1:", COUNT1, "COUNT2:", COUNT2)
print("KIDS1 bytes?", isinstance(KIDS1, bytes), "len:", len(KIDS1))
print("KIDS2 bytes?", isinstance(KIDS2, bytes), "len:", len(KIDS2))
contents = template % fmt
